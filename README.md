# Hi, I'm Andrés López Echeverri 👋

**Data Scientist · ML Engineer · Scientific Computing**  
📍 Envigado, Colombia · 🌐 Open to remote worldwide

---

## About me

I'm an Astronomy graduate (Universidad de Antioquia) with hands-on experience building high-performance data pipelines, training machine learning models, and processing large-scale scientific simulations. My research involved analyzing cosmological N-body simulations with ~1 billion particles using IllustrisTNG, where I optimized Python/C pipelines that reduced processing runtime by ~90%.

I combine a rigorous quantitative background with practical skills in Python, C/C++, SQL, and deep learning, and 5+ years of remote professional experience working in English for a U.S.-based platform.

---

## 🛠 Tech Stack

**Languages**  
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![C](https://img.shields.io/badge/C-00599C?style=flat&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=postgresql&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=flat&logo=gnubash&logoColor=white)

**ML & Deep Learning**  
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-F7931E?style=flat&logo=scikitlearn&logoColor=white)
![XGBoost](https://img.shields.io/badge/XGBoost-FF6600?style=flat&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat&logo=tensorflow&logoColor=white)
![LSTM](https://img.shields.io/badge/LSTM-blueviolet?style=flat)
![Transformers](https://img.shields.io/badge/Transformers-FFD21E?style=flat&logo=huggingface&logoColor=black)

**Data & Scientific Computing**  
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white)
![SciPy](https://img.shields.io/badge/SciPy-8CAAE6?style=flat&logo=scipy&logoColor=white)
![HDF5](https://img.shields.io/badge/HDF5-E70200?style=flat&logoColor=white)
![Astropy](https://img.shields.io/badge/Astropy-4B0082?style=flat&logoColor=white)

**Tools**  
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat&logo=jupyter&logoColor=white)

---

## 🚀 Featured Projects

### [💹 Currency Price Direction Prediction (EUR/USD)](https://github.com/AndresL7/ForexPredictML)
End-to-end ML pipeline to predict next-day EUR/USD price direction from historical OHLCV data.  
- Feature engineering: technical indicators, lag features  
- Models: XGBoost, logistic regression, hyperparameter tuning  
- **62% directional accuracy** on out-of-sample data  
`Python` `XGBoost` `Pandas` `Scikit-learn`

---

### 📉 Customer Churn Prediction
Machine learning project to predict customer churn and support proactive retention strategies.  
- Data preprocessing and feature engineering for customer behavior data  
- Model training and evaluation for churn classification  
- Performance tracking with precision, recall, F1-score, and ROC-AUC  
`Python` `Scikit-learn` `Pandas` `Classification`

---

### [🌊 2D Fluid Simulation — Smoothed Particle Hydrodynamics (SPH)](https://github.com/AndresL7/WaterDischarge)
From-scratch implementation of a 2D SPH fluid dynamics simulation in C.  
- Models pressure, viscosity, and surface tension forces  
- Grid-based linked-list nearest-neighbor search reducing O(n²) to near-linear cost  
- Real-time simulation performance  
`C` `Numerical Methods` `Scientific Computing`

---

### 🌌 Cosmological N-body Pipeline (Research — IllustrisTNG)
Data analysis and pipeline engineering for my undergraduate dissertation on galaxy evolution.  
- Processed ~1 billion particle simulations from IllustrisTNG  
- Optimized Python/C pipelines: **~90% reduction in runtime**  
- Statistical modeling of halo environments, mass accretion, and galaxy morphology  
`Python` `C` `HDF5` `HPC` `Astropy` `SciPy`

---

## 📊 Research

**BSc Dissertation** · Universidad de Antioquia · Mar 2024 – Jan 2026  
*Environmental Effects on Mass Accretion & Galactic Morphology*  
Advisor: Juan Carlos Muñoz-Cuartas  
Analyzed cosmological simulations to study the relationship between halo environment, mass accretion history, and galaxy morphology using IllustrisTNG data.

---

## 🎓 Education & Certifications

🎓 **BSc Astronomy** — Universidad de Antioquia · GPA 4.44/5.00 
📜 Introduction to Data Science in Python — *University of Michigan*  
📜 Introduction to SQL — *University of Michigan*  
📜 Agentic AI and AI Agents — *Vanderbilt University*  
📜 Harnessing Data for Scientific Discovery - A Deep Dive into Data Analysis and ML for Physical and Life Sciences - *UIDE (Ecuador)*

📜 Summer Astrophysics School — *IRyA, UNAM (Mexico)*

---

## 🌍 Languages

🇪🇸 Spanish — Native · 🇬🇧 English — Fluent · 🇩🇪 German — Intermediate

---

## 📫 Contact

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/andres-lopez-e)
[![Email](https://img.shields.io/badge/Email-EA4335?style=flat&logo=gmail&logoColor=white)](mailto:andres.lopeze1@udea.edu.co)
